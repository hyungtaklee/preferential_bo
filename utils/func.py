...
class test_func(metaclass=ABCMeta):
    ...
    def costs(self, input, fidelity=None):
        raise NotImplementedError(f"Costs are not defined for {type(self).__name__}.")
...
class Beale(test_func):
    ...
    def values(self, input, fidelity=None):
        ...
        if fidelity == 0:
            ...
            raise ValueError(
                f"Unsupported fidelity {fidelity} for {type(self).__name__}; "
                f"expected 0 <= fidelity < {self.M}."
            )
...
class CurrinExp(test_func):
    ...
    def _high_fidelity_values(self, input):
        y = input[:, 1]
        FormerTerm = np.ones_like(y, dtype=float)
        positive = y > 0
        FormerTerm[positive] = 1 - np.exp(-1 / (2.0 * y[positive]))
        LatterTerm = (
